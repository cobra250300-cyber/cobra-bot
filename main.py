...
# معرف المطور الجديد (تم التحديث بناءً على طلبك)
...
def rate_limit(func):
    ...
    async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if not rate_limiter.is_allowed(user_id):
            await update.message.reply_text("⏰ **نظام الحماية:** أنت سريع جداً! انتظر دقيقة.")
            return
...
async def get_ai_response(text):
    ...
    except: return "📡 فشل الاتصال بالقاعدة السحابية."
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome = (
        ...
        "🎨 `/gen` : توليد صور (بدون شعار).\n"
        "📱 `/no` : استخراج 100 رقم Wecom.\n"
        "💳 `/cod` : بيانات فيزا وهمية للتعليم.\n"
        "🔊 `/voice` : تحويل النص لصوت هاكر.\n"
    )
    keyboard = [
        [InlineKeyboardButton("📱 أرقام", callback_data='cmd_no'), InlineKeyboardButton("💳 فيزا", callback_data='cmd_cod')],
...
async def generate_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_prompt:
        await update.message.reply_text("❌ أرسل وصفاً بعد الأمر. مثال: `/gen هاكر`")
        return
...
async def visa_gen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💳 **بيانات وهمية (للتعليم فقط):**\n\n" + "\n".join(cards), parse_mode='Markdown')
...
async def text_to_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    else: await update.message.reply_text("❌ اكتب نصاً بعد الأمر.")
...
if __name__ == '__main__':
    ...
    print(f"🚀 Cobra V10 is running! Admin ID: {DEVELOPER_ID}")
    app.run_polling()
